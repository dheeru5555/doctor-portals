...
const DialogTitle = withStyles(styles)((props) => {
  const {
    children, classes, handleNextDay, currentDate, vitalDetails, handleVitalAdd,
...
function createData(id, name, range, unit, color, status) {
  return {
    id,
    name,
    range,
...
function Vitals(props) {
  const {
    classes, isProfile, waitListData, patientId
  } = props;
    ...
    api.ACCOUNTS_URI().get('patients/getVitalsByDate', { params })
      .then((resp) => {
        if (resp.data.patient_vital.length > 0) {
          ...
          setPatientVitals(respVital.sort((a, b) => parseFloat(a.severity) - parseFloat(b.severity)));
        }
      })
  ...
  const handleVitalAdd = async (event, greenLcl, greenUcl, orangeLcl, orangeUcl, redLcl, redUcl) => {
    ...
    await api.ACCOUNTS_URI().post('patients/addEditVitals', inputParams)
      .then((addVitalResp) => {
        // const getIndex = newArr.findIndex((data) => data.id === id);
        // newArr[getIndex] = {
        //     ...newArr[getIndex],
        //     value: value
        // };
        setIsUpdated(!isUpdated);
        // setPatientVitals([...patientVitals, newArr])
      })
