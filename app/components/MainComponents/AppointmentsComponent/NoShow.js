...
import { fetchNoshowAppointment, setSelectedQueueId } from '../../../redux/actions/appointmentAction';
...
function Noshow(props) {
  const api = new API();
  // React.useEffect(() => {
  //   if (props.noshowAppointments === null) {
  ...
  React.useEffect(() => {
    if (
      ...
      && (props.noshowAppointments !== null)
      && (props.noshowAppointments.length > 0)
    ) {
      const bookedAppointmentsData = props.noshowAppointments.map((bAppointment) => {
  ...
  const handleRequestSort = () => {
    const intermediateTodayBooking = { ...noShowBooking };
    intermediateTodayBooking.sortorder = (noShowBooking.sortorder === 'asc') ? 'desc' : 'asc';
    setNoShowBooking(intermediateTodayBooking);
  };
  ...
  const updateAppointmentStatus = async (appId, appStatusId) => {
    await api.ACCOUNTS_URI().post('appointments/updateAppointmentStatus', {
      ...
      setSnackBarInfo({
        isSnackBarOpen: true,
        snackBarText: `Internal Server Error ${err}`,
        snackBarSeverity: 'error',
      });
    });
  };

  const renderActionButtons = (appId, appType, consultationStart, cancelledAt) => {
    if (appType === 'Walk-In') {
      if (cancelledAt !== null) {
        return (
  ...
  React.useEffect(() => {
    // props.fetchNoshowAppointment(noShowBooking);
  ...
  const fetchNoShowAppointments = async () => {
    const defaultValue = {
      sortby: 'patient_name',
      sortorder: 'desc',
      page: 1,
    ...
    if (JSON.stringify(defaultValue) !== JSON.stringify(noShowBooking)) {
    }
    setIsLoading(true);
    await props.fetchNoshowAppointment(noShowBooking).finally(() => setIsLoading(false));
  };
          ...
          <Paper className={classes.rootTable} elevation={0}>
            {toolbar && (
              <EnhancedTableToolbar
                ...
                title="No Show"
                placeholder="Search"
                bookingTypes={noShowBooking}
                handleCheckboxChange={handleCheckboxChange}
                fetchAppointments={fetchNoShowAppointments}
              />
            )}
              ...
              <TableContainer className={classes.container}>
                ...
                >
                  <EnhancedTableHead
                    numSelected={selected.length}
                    order={order}
                    orderBy={noShowBooking.sortorder}
                  ...
                  {isLoading ? (
                    ...
                    <>
                      {((props.noshowAppointments === undefined)
                        || (props.noshowAppointments === null)
                        || (props.noshowAppointments.length === 0)) ? (
                        <TableBody>
                          <TableRow>
                              ...
                              >
                                ...
                                <TableCell align="right">
                                  {!isFrontdesk && (
                                    <Button
                                      variant="contained"
                                      onClick={async () => await handleQuickConsultationClick(n.id)}
                                ...
                                {/* {renderActionButtons(n.id, n.appointmentType, n.consultationStart, n.canceledAt)} */}
                              </TableRow>
                            );
                          })}
                          {emptyRows > 0 && (
            ...
            {pagination && (
              ...
              // />
              <>
                {props.noshowAppointmentsAllData !== null ? (
                  <Pagination count={props.noshowAppointmentsAllData.last_page} className={classes.pagination} page={pageNum} onChange={handlePagination} />
                ) : ''}
              </>
            )}
          </Paper>
...
const mapStateToProps = state => ({
  noshowAppointments: state.get('appointmentReducer').noshowAppointments,
  noshowAppointmentsAllData: state.get('appointmentReducer').noshowAppointmentsAllData,
  masterData: state.get('dashboardReducer').masterData,
  imagesOrDocs: state.get('dashboardReducer').imagesOrDocs,
});

export default connect(mapStateToProps, { fetchNoshowAppointment, setSelectedQueueId })(withStyles(styles)(injectIntl(Noshow)));
