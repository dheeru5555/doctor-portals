...
import { fetchPausedAppointments, setSelectedQueueId } from '../../../redux/actions/appointmentAction';
...
function Paused(props) {
  ...
  const storedClinics = localStorage.getItem('selectedClinics');
  const selectedClinics = storedClinics !== '' ? JSON.parse(storedClinics) : [];

  const [isLoading, setIsLoading] = useState(true);
  const [pageNum, setPageNum] = useState(1);

  const [pausedBooking, setPausedBooking] = useState({
    sortby: 'patient_name',
    sortorder: 'desc',
    page: pageNum,
    length: 10,
    ...
    appointment_type_id: [],
    consultation_type: [],
    clinic_id: selectedClinics !== '' && selectedClinics.length == 0 ? [] : selectedClinics.map((clinic) => clinic.id),
    cr_id: (localStorage.getItem('cr_id') !== null && localStorage.getItem('cr_id') !== undefined) ? parseInt(localStorage.getItem('cr_id')) : ''
  });

  const handlePagination = (event, value) => {
    setPageNum(value);
    pausedBooking.page = value;
  };

  React.useEffect(() => {
    if (
      (props.masterData !== null)
      && (props.pausedAppointments !== null)
      && (props.pausedAppointments.length > 0)
    ) {
      const bookedAppointmentsData = props.pausedAppointments.map((bAppointment) => {
        ...
        const pausedAt = (bAppointment.updated_at !== null) ? helper.tConvert((bAppointment.updated_at).substring(0, bAppointment.updated_at.length - 3)) : '';
  ...
  const handleRequestSort = () => {
    const intermediateTodayBooking = { ...pausedBooking };
    intermediateTodayBooking.sortorder = (pausedBooking.sortorder === 'asc') ? 'desc' : 'asc';
    setPausedBooking(intermediateTodayBooking);
  };
  ...
  const fetchPausedAppointments = async () => {
    ...
    if (JSON.stringify(defaultValue) !== JSON.stringify(pausedBooking)) {
    }
    setIsLoading(true);
    await props.fetchPausedAppointments(pausedBooking).finally(() => setIsLoading(false));
  };
          ...
          <Paper className={classes.rootTable} elevation={0}>
            {toolbar && (
              <EnhancedTableToolbar
                ...
                title="Paused Consultations"
                placeholder="Search"
                bookingTypes={pausedBooking}
                handleCheckboxChange={handleCheckboxChange}
                fetchAppointments={fetchPausedAppointments}
              />
            )}
              ...
              <TableContainer className={classes.container}>
                ...
                >
                  <EnhancedTableHead
                    numSelected={selected.length}
                    order={order}
                    orderBy={pausedBooking.sortorder}
                  ...
                  {isLoading ? (
                    ...
                    <>
                      {((props.pausedAppointments === undefined)
                        || (props.pausedAppointments === null)
                        || (props.pausedAppointments.length === 0)) ? (
                        <TableBody>
                          <TableRow>
                          ...
                          {data.slice(page * rowsPerPage, (page * rowsPerPage) + rowsPerPage).map((n, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <TableCell align="left">{n.pausedAt}</TableCell>
                                <TableCell align="left">
                                  <p>
                                    <b>Booking ID: </b>
                                ...
                                <TableCell align="right">
                                  {!isFrontdesk && (
                                    <Button variant="contained" className={classes.button} onClick={async () => await handleResumeConsultation(n.id)}>
                                      Resume Consultation
            ...
            {pagination && (
              ...
              // />
              <>
                {props.pausedAppointmentsAllData !== null ? (
                  <Pagination count={props.pausedAppointmentsAllData.last_page} className={classes.pagination} page={pageNum} onChange={handlePagination} />
                ) : ''}
              </>
            )}
          </Paper>
...
const mapStateToProps = state => ({
  pausedAppointments: state.get('appointmentReducer').pausedAppointments,
  pausedAppointmentsAllData: state.get('appointmentReducer').pausedAppointmentsAllData,
  masterData: state.get('dashboardReducer').masterData,
  imagesOrDocs: state.get('dashboardReducer').imagesOrDocs,
});

export default connect(mapStateToProps, { fetchPausedAppointments, setSelectedQueueId })(withStyles(styles)(injectIntl(Paused)));
